...
Data_inv <- function(name, waterbod_name){
  ...
  `Content Description` = c(
    sprintf('The BeaverNetwork dataset for the %s of England region in 
    ...
    gradient and stream width.', name),
    ...
    sprintf('Fine (High) resolution (10 m) Beaver Habitat Index for the %s region. in 
    Geotiff (.tif) format.', name),
    sprintf('Coarse (Low) resolution (1 km) Beaver Habitat Index for the %s region. in 
    Geotiff (.tif) format.', name)
    )
  )
